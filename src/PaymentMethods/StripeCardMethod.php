 ...
 *
 * Registered by the host app, never automatically: a store that does not take
 * card through Stripe simply never registers it. It lives here rather than in
 * lunarphp/stripe because it extends this package's AbstractPaymentMethod, and
 * no package in the Lunar monorepo may depend on this one. It needs nothing
 * from lunarphp/stripe itself, naming the gateway only through its driver
 * handle and its own config, so it works whether or not that package is
 * installed.
 */
class StripeCardMethod extends AbstractPaymentMethod implements ExplainsUnavailability
{
     ...
     * currency through `lunar.checkout.stripe.minimum_amounts`.
     *
     ...
     * (spec 0002 section B). A zero total is left alone: nothing is charged and
     * the checkout's own zero-total handling applies.
     */
    ...
    public function minimumAmount(Cart $cart): int
    {
        ...
        $overrides = (array) config('lunar.checkout.stripe.minimum_amounts', []);
